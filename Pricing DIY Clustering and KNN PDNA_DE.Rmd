...
clid <- 443 # Either choose 443 for Sofa & Console Tables or 6289 for nightstands
...
df_ss_query_2 <- paste0("SELECT * FROM
csn_junk..tblDailySnapshot_UK
WHERE clid IN (", clid,")")

# Execute query
dbHandle <- odbcDriverConnect('driver=SQL Server;server=BOSQLINT02;trusted_connection=true')
df_ss_UK_forPDNA <- sqlQuery(dbHandle, df_ss_query_2, stringsAsFactors = FALSE)
odbcClose(dbHandle)

# Return results
head(df_ss_UK_forPDNA, 10)
...
material_func <- function(SKU_list){
  material_query <- paste0("DECLARE @BclgID INT = 3
  SELECT
